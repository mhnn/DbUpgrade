    ...
    internal partial class Migration(IOptions<AppConfig> config, HospitalIDProcessor hospitalIDProcessor, ReplaceLineEndingsProcessor replaceLineEndingsProcessor)
    {
        ...
        private void BuildEngines()
        {
            ...
            foreach (var conn in conns)
            {
                ...
                {
                    HtmlReportBuilder.Build(engine, conn.Key.ToString(), VERSION);
        ...
        private void PerformUpgrade()
        {
            ...
            for (var i = 0; i < _upgradeEngines.Count; i++)
            {
                ...
                ConsoleUtil.WriteLine($"第{i + 1}个数据库{key}开始升级，预计将执行{upgradeEngine.GetScriptsToExecute().Count}个脚本", ConsoleColor.DarkCyan);

                var result = upgradeEngine.PerformUpgrade();
                if (!result.Successful)
                {
                    ConsoleUtil.WriteLine($"数据库{key}升级失败", ConsoleColor.Red);
                    ConsoleUtil.WriteLine($"执行脚本{result.ErrorScript.Name}失败！", ConsoleColor.Red);
                    Console.WriteLine(result.Error);
                    return;
                }
            }
            ConsoleUtil.WriteLine("所有数据库升级完成", ConsoleColor.DarkGreen);
        }
        #endregion
    }
}
